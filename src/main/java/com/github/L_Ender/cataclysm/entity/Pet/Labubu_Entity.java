...
public class Labubu_Entity extends AnimationPet {

    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(Labubu_Entity.class, EntityDataSerializers.INT);
    ...
    public Labubu_Entity(EntityType<? extends TamableAnimal> entityType, Level level) {
        super(entityType, level);
    }

    public static AttributeSupplier.Builder bakeAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, 10.0D)  // Slightly more health than teddy bear
                .add(Attributes.ATTACK_DAMAGE, 3.0D)  // Slightly stronger
                .add(Attributes.ATTACK_SPEED, 1.0D)
                .add(Attributes.MOVEMENT_SPEED, 0.32D)  // Slightly faster
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        // Server side logic
        if (this.isTame()) {
            // First priority: Healing with food when labubu is hurt
            ...
            if (this.isOwnedBy(player)) {
                ...
                if (shouldToggleSit) {
                    ...
                    if (newSitState) {
                        player.displayClientMessage(net.minecraft.network.chat.Component.literal("Labubu is now sitting"), true);
                    } else {
                        player.displayClientMessage(net.minecraft.network.chat.Component.literal("Labubu is now standing"), true);
                    }
            ...
            if (this.random.nextInt(3) == 0) {
                this.tame(player);
                // Scale health up when tamed (like wolves: 10 HP -> 45 HP)
    ...
    public boolean isFood(ItemStack itemStack) {
        // Labubu is tamed with precious gems
        return itemStack.is(Items.EMERALD) || itemStack.is(Items.DIAMOND);
    }

    @Nullable
    @Override
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageableMob) {
        Labubu_Entity baby = ModEntities.LABUBU.get().create(serverLevel);
        if (baby != null) {
            // Baby inherits random variant from parents
            if (ageableMob instanceof Labubu_Entity otherParent) {
    ...
    protected SoundEvent getAmbientSound() {
        return SoundEvents.NOTE_BLOCK_CHIME.get();  // More mystical sound
    }
    ...
    public static boolean canSpawn(EntityType<Labubu_Entity> entityType, LevelAccessor levelAccessor, MobSpawnType spawnType, BlockPos pos, RandomSource random) {
        return Animal.checkAnimalSpawnRules(entityType, levelAccessor, spawnType, pos, random) && random.nextInt(10) == 0;  // Slightly rarer
    }
