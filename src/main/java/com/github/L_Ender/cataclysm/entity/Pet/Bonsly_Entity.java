...
public class Bonsly_Entity extends LLibraryAnimationPet implements ISemiAquatic, Bucketable {
    private static final EntityDataAccessor<Boolean> FROM_BUCKET = SynchedEntityData.defineId(Bonsly_Entity.class, EntityDataSerializers.BOOLEAN);

    public static final Animation BONSLY_BITE = Animation.create(14);
    public static final Animation BONSLY_ABYSS_BLAST = Animation.create(157);
    public float sitProgress;
    public float prevSitProgress;
    public float SwimProgress;
    public float prevSwimProgress;
    private int foodFeedings;
    ...
    public Bonsly_Entity(EntityType type, Level world) {
        super(type, world);
        this.xpReward = 0;
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new BonslyAttackGoal(this));
        this.goalSelector.addGoal(0, new BonslyBiteAttackGoal(this, BONSLY_BITE));
        ...
        this.targetSelector.addGoal(3, new EntityAINearestTarget3D(this, LivingEntity.class, 120, false, true, ModEntities.buildPredicateFromTag(ModTag.BABY_LEVIATHAN_TARGET)) {
            public boolean canUse() {
                return Bonsly_Entity.this.getCommand() != 2 && !Bonsly_Entity.this.isSitting() && super.canUse();
            }
        });
    ...
    private void switchNavigator(boolean onLand) {
        if (onLand) {
            ...
            this.moveControl = new BonslyMoveController(this, 3F, 1F, 10f);
    ...
    public void saveToBucketTag(@Nonnull ItemStack bucket) {
        ...
        compound.put("BonslyData", platTag);
    }

    @Override
    public void loadFromBucketTag(CompoundTag p_148832_) {
        Bucketable.loadDefaultDataFromBucketTag(this, p_148832_);
        if (p_148832_.contains("BonslyData")) {
            this.readAdditionalSaveData(p_148832_.getCompound("BonslyData"));
        }
    }
    ...
    public ItemStack getBucketItemStack() {
        ItemStack stack = new ItemStack(ModItems.BONSLY_BUCKET.get());
        return stack;
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!isTame() && itemstack.is(Items.WHEAT_SEEDS)) {
            ...
            foodFeedings++;
            if ((foodFeedings > 10 && getRandom().nextInt(6) == 0 || foodFeedings > 30) && !net.minecraftforge.event.ForgeEventFactory.onAnimalTame(this, player)) {
                this.tame(player);
    ...
    public void aiStep() {
        ...
        if (this.getAnimation() == BONSLY_BITE) {
            if (this.getAnimationTick() == 7) {
                this.playSound(SoundEvents.PLAYER_ATTACK_SWEEP, 0.5f, 2.0f);
            }
    ...
    private void biteattack(float radius, double inflateX, double inflateY, double inflateZ) {
        ...
        if (!level().isClientSide) {
            ...
            for (LivingEntity target : hit) {
                if (!isAlliedTo(target) && !(target instanceof Bonsly_Entity) && target != this) {
    ...
    public Animation[] getAnimations() {
        return new Animation[]{NO_ANIMATION, BONSLY_ABYSS_BLAST, BONSLY_BITE};
    }
    ...
    static class BonslyBiteAttackGoal extends PetSimpleAnimationGoal<Bonsly_Entity> {

        public BonslyBiteAttackGoal(Bonsly_Entity entity, Animation animation) {
    ...
    static class BonslyBlastAttackGoal extends PetSimpleAnimationGoal<Bonsly_Entity> {

        public BonslyBlastAttackGoal(Bonsly_Entity entity, Animation animation) {
            super(entity, animation);
            this.setFlags(EnumSet.of(Flag.MOVE, Flag.LOOK, Flag.JUMP));
        }
    ...
    class BonslyAttackGoal extends Goal {
        private final Bonsly_Entity mob;
        ...
        public BonslyAttackGoal(Bonsly_Entity mob) {
            this.mob = mob;
        ...
        public void start() {
            this.mob.mode = Bonsly_Entity.AttackMode.CIRCLE;
            circlingTime = 0;
            MeleeModeTime = 0;
        ...
        public void tick() {
            ...
            if (target != null) {
                if (this.mob.mode == Bonsly_Entity.AttackMode.CIRCLE) {
                    circlingTime++;
                    circleEntity(target, circleDistance, 1.0f, clockwise, circlingTime, 0, 1);
                    if (0 >= this.mob.blast_cooldown) {
                        this.mob.mode = Bonsly_Entity.AttackMode.RANGE;
                    } else {
                        this.mob.mode = Bonsly_Entity.AttackMode.MELEE;
                    }
                } else if (this.mob.mode == Bonsly_Entity.AttackMode.RANGE) {
                    if (this.mob.getRandom().nextFloat() * 100.0F < 3f) {
                        this.mob.setAnimation(BONSLY_ABYSS_BLAST);
                    }
                } else if (this.mob.mode == Bonsly_Entity.AttackMode.MELEE) {
                    MeleeModeTime++;
                    ...
                    if(MeleeModeTime >= MELEE_MODE_TIME) {
                        ...
                        this.mob.setAnimation(BONSLY_BITE);
                    }
                }
            }
        }
    ...
    static class BonslyMoveController extends MoveControl {
        private final Bonsly_Entity entity;
        private final float speedMulti;
        private final float ySpeedMod;
        private final float yawLimit;
        private  int stillTicks = 0;
        
        public BonslyMoveController(Bonsly_Entity entity, float speedMulti, float ySpeedMod, float yawLimit) {
            super(entity);
            this.entity = entity;
